...
def chronogan(ori_data, parameters, num_samples):
    ...
    def compute_ts_structure(X_mb, X_hat):
        b = tf.shape(X_mb)[0]
        ...
        def calculate_skewness(data, axis=1):
            ...
            skewness = tf.reduce_sum(((data - mean) / (std_dev + 1e-12)) ** 3, axis=axis) * (
                Nn / ((Nn - 1) * (Nn - 2) + 1e-12)
            )
            return skewness
        ...
        ts_structure = (
            mean_weighted_average_mse
            + std_weighted_average_mse
            + mean_slope_mse
            + std_slope_mse
            + 0.5 * mean_median_mse
            + 0.5 * std_median_mse
    ...
    for itt in range(int(iterations * 0.5)):
        ...
        X_mb_np, T_mb = batch_generator(ori_data, ori_time, batch_size)
        X_mb_np = safe_np(X_mb_np)
        step_d_ae_loss = tf.constant(0.0, dtype=tf.float32)
        ...
        if X_mb_np is not None:
            ...
            if any(g is not None for g in grads):
                ...
                step_d_ae_loss = D_ae_loss_real

        if (itt % (int(iterations * 0.5) // 10 if iterations * 0.5 >= 10 else 1) == 0) or (
            itt == int(iterations * 0.5) - 1
        ):
            tf.print(
                "step:",
                itt * 2,
    ...
    for itt in range(iterations):
        ...
        if (itt % (int(iterations) // 10 if iterations >= 10 else 1) == 0) or (itt == iterations - 1):
            tf.print("step:", itt, "/", iterations, ", S_loss:", G_loss_S)
    ...
    for itt in range(iterations):
        for _ in range(2):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                ts_structure = compute_ts_structure(X_mb, X_hat)

                G_loss = (
                    (G_loss_U_ae + gamma * G_loss_U_ae_e)
            ...
            with tf.GradientTape() as tape2:
                ...
                E_loss_U = bce(tf.ones_like(Y_ae_fake), Y_ae_fake)
                E_loss = 10.0 * tf.sqrt(
                    tf.maximum(E_loss_T00 + 0.001 * 0.1 * E_loss_U, 1e-12)
                ) + 0.1 * G_loss_S
        ...
        # Recompute inside the tape so gradients are valid.
        with tf.GradientTape() as tape3:
            Y_real_2 = ae_disc(X_mb, T_mb_tf, training=True)
            Y_fake_2 = ae_disc(X_hat, T_mb_tf, training=True)
            Y_fake_3 = ae_disc(X_tilde_fake_second, T_mb_tf, training=True)

            D_loss_real_2 = bce(tf.ones_like(Y_real_2), Y_real_2)
            D_loss_fake_2 = bce(tf.zeros_like(Y_fake_2), Y_fake_2)
            D_loss_fake_3 = bce(tf.zeros_like(Y_fake_3), Y_fake_3)

            D_ae_loss_second = D_loss_real_2 + beta * (D_loss_fake_2 + gamma * D_loss_fake_3)

        vars_d = ae_disc.trainable_variables
        grads = tape3.gradient(D_ae_loss_second, vars_d)
        step_d_ae_loss_second = tf.constant(0.0, dtype=tf.float32)

        if any(g is not None for g in grads):
            grads = [g if g is not None else tf.zeros_like(v) for g, v in zip(grads, vars_d)]
            update_second = True
            if tf.executing_eagerly():
                update_second = float(D_ae_loss_second.numpy()) > 0.15

            if update_second:
                D_ae_second_optimizer.apply_gradients(zip(grads, vars_d))
                step_d_ae_loss_second = D_ae_loss_second

        if (itt % (int(iterations) // 10 if iterations >= 10 else 1) == 0) or (itt == iterations - 1):
            tf.print("step:", itt, "/", iterations, ", D_loss:", step_d_ae_loss_second)
        ...
        if (itt >= int(iterations * 0.5)) and (itt % 500 == 0 or itt == iterations - 1):
            ...
            generated_data_curr = recovery(
                supervisor(
                    generator(
                        Z_mb_t,
                        tf.convert_to_tensor(np.asarray(ori_time, dtype=np.int32)),
                        training=False,
                    ),
                    tf.convert_to_tensor(np.asarray(ori_time, dtype=np.int32)),
                    training=False,
            ...
            filtered_discriminative_score = (
                discriminative_score[
                    (discriminative_score <= np.percentile(discriminative_score, 75))
                ]
                if discriminative_score.size > 0
                else np.array([])
            )
            ...
            filtered_predictive_score = (
                predictive_score[(predictive_score <= np.percentile(predictive_score, 75))]
                if predictive_score.size > 0
                else np.array([])
            )
